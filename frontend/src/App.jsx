...
import { LayoutDashboard, UserRound, FileText, BriefcaseBusiness, Code2, Target, Menu, Sparkles, Search, CheckCircle2, AlertTriangle, ArrowRight, Moon, Sun, Zap, TrendingUp } from "lucide-react";
...
export default function App() {
  ...
  return <div className={dark ? "app dark" : "app"}>
    ...
    <main className="main"><header><button className="mobile-menu"><Menu/></button><div><p className="eyebrow">CAREER COMMAND CENTER</p><h1>{active}</h1></div><div className="header-actions"><div className="status"><span/> Career journey active</div><button className="theme-toggle" onClick={() => setDark(!dark)} aria-label="Toggle dark mode">{dark ? <Sun size={16}/> : <Moon size={16}/>}</button></div></header>{active === "Job Analyzer" ? <JobAnalyzer/> : <Dashboard setActive={setActive}/>}</main>
  </div>;
}
